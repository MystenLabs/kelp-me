...
const CommitPage = () => {
  ...
  const [currentKelpOwner, setCurrentKelpOwner] = useState("");

  const handleKelpAddressChange = (e: ChangeEvent<HTMLInputElement>) => {
    setKelpAddress(e.target.value);
  };

  const handleCurrentKelpOwnerChange = (e: ChangeEvent<HTMLInputElement>) => {
    setCurrentKelpOwner(e.target.value);
  };
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-semibold mb-4 text-center">Submit Commit</h2>

      <div className="mb-4">
        <label
          className="block text-gray-700 font-medium mb-2"
          htmlFor="currentKelpOwner"
        >
          Current Kelp Owner
        </label>
        <input
          id="currentKelpOwner"
          type="text"
          value={currentKelpOwner}
          onChange={handleCurrentKelpOwnerChange}
          min="1"
          className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="Enter Current Kelp Owner"
        />
      </div>
