...
module kelp::kelp {
    ...
    const VERSION: u8 = 0;
    /// Minimum commit fee required.
    const COMMIT_FEE: u64 = 10_000_000_000;
    /// Duration of the reveal window in milliseconds (1 Hour).
    const REVEAL_WINDOW: u64 = 3_600_000;
    ...
    public fun reveal(
        ...
        let expected_hash = hash(
            &bcs::to_bytes(
                &vector[
                    *address_c.to_string().as_bytes(),
                    *claimant.to_string().as_bytes(),
                    nonce
                ]
            )
        );
